...
class IntradayDataHandler:
    ...
    def fetch_2min_data(self):
        ...
        for ticker in self.tickers:
            ...
            time.sleep(0.09)  # To avoid rate-limiting
    ...
    def load_process_intraday_data(self):
        ...
        # ✅ Report NaNs for intraday files
        if nan_files:
    ...
    def append_intraday_data(self):
        ...
        for file in os.listdir(self.raw_folder):
            if file.endswith('.csv'):
                ...
                # ✅ Ensure only intraday timeframes are processed
                if timeframe not in self.intraday_timeframes:
                    continue
                
                raw_path = os.path.join(self.raw_folder, file)
                processed_path = os.path.join(self.processed_folder, file)
                # final tranformed data stored in tranf_folder
                ...
                try:
                    ...
                    # ✅ Ensure date and time uniqueness
...
if __name__ == "__main__":
    ...
    collector = IntradayDataHandler(ticker, raw_folder, processed_folder)
    collector.fetch_intraday_data()
